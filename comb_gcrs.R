...
rankmap_files <- list.files(parent_dir_data, recursive = TRUE, pattern = "_result.tif", full.names = TRUE)
...
out_dir <- "C:/Users/ogundipe/Documents/OneDrive - Eidg. Forschungsanstalt WSL/visua/new_data/com_results/com_png/"
...
library(terra)
hh=rast("C:/Users/ogundipe/Documents/OneDrive - Eidg. Forschungsanstalt WSL/visua/new_data/PAR_75/1_1_1.tif.tif")
#Tota current pa
...
for (data_file in rankmap_files) {
               ...
               ss_pa_files <- list.files(parent_dir_sspa, pattern = paste0(data_figures, ".tif",".tif"), full.names = TRUE)
               ...
               if (length(ss_pa_files) > 0) {
                              ...
                              png(filename = paste0(out_dir, data_figures, ".png"), width = 17, height = 30, unit = "cm", res = 300, pointsize = 7.5)
                              
                              # Plot the rank map
                              plot(rankmap, col = clsi, legend = FALSE, axes = FALSE)
...
raster_dir <-"C:/Users/ogundipe/Documents/OneDrive - Eidg. Forschungsanstalt WSL/visua/new_data/PAR_75"

# List all raster files in the directory
raster_files <- list.files(raster_dir, pattern = ".tif$", full.names = TRUE)

# CRS to assign (e.g., EPSG code)
crs_to_assign <- "epsg:21781"  # Replace with your desired CRS

# Loop over each raster file and assign the CRS
for (file in raster_files) {
               # Read the raster file
               raster_obj <- rast(file)
               
               # Assign the CRS
               crs(raster_obj) <- crs_to_assign
               
               # Construct the output filename with the assigned CRS
               out <- file.path("C:/Users/ogundipe/Documents/OneDrive - Eidg. Forschungsanstalt WSL/visua/new_data/PAR_75/crs", paste0(basename(file), ".tif"))
               
               # Save the raster file with the assigned CRS
               writeRaster(raster_obj, filename = out, overwrite = TRUE)
}
