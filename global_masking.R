...
library(sf)
#library(raster)
output="C:/Users/ogundipe/Documents/OneDrive - Eidg. Forschungsanstalt WSL/visua/new_data/masked_areas/glo_masked"
...
for (path in rankmap_files) {
               ...
               output_path <- file.path(output, paste0("maskedglob_", identifier, ".tif"))
...
masked_files <- list.files(parent_dir, pattern = "maskedglob_.*\\.tif$", full.names = TRUE)
...
for (path in masked_files) {
               ...
               identifier <- gsub('.*/maskedglob_(\\d+_\\d+_\\d+).*', '\\1', path)
               output_path <- file.path(parent_dir, paste0("scaledglo_", identifier, ".tif"))
